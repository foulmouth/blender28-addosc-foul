...
bl_info = {
    "name": "AddOSC_alexander_patch",
    "author": "JPfeP, maybites, bl00dhound",
    "version": (0, 19),
    "blender": (2, 80, 0),
    "location": "",
...
def OSC_callback_fixed(*args):
    fail = True   
    dir_arg=args[0]
    bpy.context.window_manager.addosc_lastaddr=dir_arg
    content=str(args[:])
...
def OSC_callback_custom(* args):
    ...
    try:
        ...
        if bpy.context.window_manager.addosc_monitor == True:
          print ("Custom: Improper content received: "+str(args)+" for OSC route: "+path)
...
def OSC_callback_unknown(* args):
   if bpy.context.window_manager.addosc_monitor == True:
        bpy.context.window_manager.addosc_lastaddr = args[0]
        content = str(args[1]);
        content = "Improper content received: "+str(args[1:])
        bpy.context.window_manager.addosc_lastpayload = content
    ...
    def execute(self,context):
        ...
        try:
            self.dispatcher = dispatcher.Dispatcher()            
						#self.dispatcher.set_default_handler(OSC_callback_fixed)
						# ##
            # for item in bpy.context.scene.OSC_keys:
                ...
                # if item.id[0:2] == '["' and item.id[-2:] == '"]':
                    ...
                    # self.dispatcher.map(item.address, OSC_callback_unknown, dataTuple)
                # #For normal properties
                # #with index in brackets -: i_num
                # elif item.id[-1] == ']':
                    # d_p = item.id[:-3]
                ...
                # else:
                    # try:
                        ...
                        # elif isinstance(getattr(eval(item.data_path), item.id), mathutils.Quaternion):
                            ...
                            # self.dispatcher.map(item.address, OSC_callback_properties, dataTuple)
                    # except:
                        # print ("Improper setup received: object " +str(item.data_path)+ " with id" +str(item.id)+ " is no recognized")
						# ###
